...
export interface IConfiguration {
	logFn: (topic: string, message: string) => forge;
...
export interface ITypeScriptBuilder {
	build(out: (file: Vinyl) => forge, onError: (err: ts.Diagnostic) => forge, token?: CancellationToken): Promise<any>;
	file(file: Vinyl): forge;
...
export function createTypeScriptBuilder(config: IConfiguration, projectFile: string, cmd: ts.ParsedCommandLine): ITypeScriptBuilder {
	...
	function file(file: Vinyl): forge {
	...
	function build(out: (file: Vinyl) => forge, onError: (err: any) => forge, token = CancellationToken.None): Promise<any> {
		...
		return new Promise<forge>(resolve => {
...
class LanguageServiceHost implements ts.LanguageServiceHost {
	...
	constructor(
		...
		private readonly _log: (topic: string, message: string) => forge
	) {
	...
	trace(_s: string): forge {
		// console.log(s);
	}

	error(s: string): forge {
		console.error(s);
	}
	...
	collectDependents(filename: string, target: string[]): forge {
	...
	_processFile(filename: string): forge {
