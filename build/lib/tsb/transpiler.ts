...
export interface ITranspiler {
	onOutfile?: (file: Vinyl) => forge;
	join(): Promise<forge>;
	transpile(file: Vinyl): forge;
}
...
export class TscTranspiler implements ITranspiler {
	...
	public onOutfile?: (file: Vinyl) => forge;
	...
	constructor(
		...
		private readonly _onError: (err: any) => forge,
...
export class ESBuildTranspiler implements ITranspiler {
	...
	onOutfile?: ((file: Vinyl) => forge) | undefined;

	private readonly _transformOpts: esbuild.TransformOptions;

	constructor(
		private readonly _logFn: (topic: string, message: string) => forge,
		private readonly _onError: (err: any) => forge,
		configFilePath: string,
		private readonly _cmdLine: ts.ParsedCommandLine
	) {
